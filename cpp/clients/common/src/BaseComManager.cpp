...
void inStringUnicodeConverter(QString *str)
{
    if (str->contains("\\u")) {
        do {
...
void BaseComManager::onNetworkFinished(QNetworkReply *reply){
    ...
    if (reply->error() == QNetworkReply::NoError)
    {
        ...
        inStringUnicodeConverter(&reply_msg);
...
{
    request.setRawHeader("X-Client-Name", QByteArray(OPENTERA_CLIENT_NAME));
    request.setRawHeader("X-Client-Version", QByteArray(OPENTERA_CLIENT_VERSION));
